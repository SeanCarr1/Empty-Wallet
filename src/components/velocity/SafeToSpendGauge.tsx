...
export const SafeToSpendGauge: React.FC<SafeToSpendGaugeProps> = ({ metrics, currency }) => {
  ...
  return (
    <View className="w-full bg-background-card rounded-3xl p-5 border border-background-border/80 shadow-sm">
      {/* Header Badge */}
      <View className="flex-row items-center justify-between mb-3">
        ...
        <View className={`flex-row items-center px-2.5 py-1 rounded-full border ${statusBg}`}>
          <StatusIcon size={12} color={isHealthy ? '#10B981' : isCaution ? '#F59E0B' : '#F43F5E'} />
          <Text className={`text-xs font-semibold ml-1.5 ${statusText}`}>
            {statusLabel}
          </Text>
        </View>
      </View>
      ...
      <View className="my-1">
        <View className="flex-row items-baseline">
          <Text className="text-4xl font-bold text-content-primary tracking-tight">
            {formatCurrency(metrics.dailyAllowance, currency)}
          </Text>
          <Text className="text-content-tertiary text-sm font-medium ml-2">/ day</Text>
        </View>
        <Text className="text-content-secondary text-xs mt-1">
          {metrics.daysRemainingInCycle} days left in current cycle
        </Text>
      </View>

      {/* Visual Budget Progress Bar */}
      <View className="mt-4">
        <View className="h-2.5 w-full bg-background-elevated rounded-full overflow-hidden flex-row">
          <View
      ...
      <View className="flex-row items-center justify-between pt-3 mt-3 border-t border-background-border/40">
        <View>
          <Text className="text-content-tertiary text-[10px] uppercase font-semibold">Remaining</Text>
        ...
        <View className="items-center">
          <Text className="text-content-tertiary text-[10px] uppercase font-semibold">Projected Month End</Text>
        ...
        <View className="items-end">
          <Text className="text-content-tertiary text-[10px] uppercase font-semibold">Status</Text>
